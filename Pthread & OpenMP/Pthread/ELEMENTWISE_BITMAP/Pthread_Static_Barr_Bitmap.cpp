...
void* transposeBitmapThreadBarr(void* arg) {
...
vector<uint32_t> transpose_bitmap_pthread_barr(vector<vector<uint32_t>> bitLists, int numThreads) {
    ...
    for (int i = 0; i < numThreads; ++i) {
        ...
        threadData[i].t_id = i;
        pthread_create(&threads[i], NULL, transposeBitmapThreadBarr, (void*)&threadData[i]);
    }
...
void* elementWiseBitmapThreadBarr(void* arg) {
...
vector<int> element_wise_bitmap_pthread_barr(vector<uint32_t> bitListsTranspose, int testNumber, int numThreads) {
    ...
    for (int i = 0; i < numThreads; ++i) {
        ...
        threadData[i].t_id = i;
        pthread_create(&threads[i], NULL, elementWiseBitmapThreadBarr, (void*)&threadData[i]);
    }
...
void* elementWiseBitmapThreadBarr_D(void* arg) {
    ThreadData2* data = (ThreadData2*)arg;
    vector<uint32_t> bitListsTranspose = data->bitListsTranspose;
...
vector<int> element_wise_bitmap_pthread_barr_D(vector<uint32_t> bitListsTranspose, int testNumber, int numThreads) {
    ...
    for (int i = 0; i < numThreads; ++i) {
        ...
        threadData[i].result = intersection;
        threadData[i].testNumber = testNumber;
        pthread_create(&threads[i], NULL, elementWiseBitmapThreadBarr_D, (void*)&threadData[i]);
    }

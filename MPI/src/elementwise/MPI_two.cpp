...
int main(int argc, char* argv[]) {
    ...
    if (world_rank != 0) {
        ...
        // 根进程接收其他进程的数据
        vector<int> global_result(local_result);

        for (int i = 1; i < world_size; ++i) {
            int remote_size;
            MPI_Recv(&remote_size, 1, MPI_INT, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            vector<int> remote_result(remote_size);
            MPI_Recv(remote_result.data(), remote_size, MPI_INT, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

            global_result.insert(global_result.end(), remote_result.begin(), remote_result.end());
        }
    }

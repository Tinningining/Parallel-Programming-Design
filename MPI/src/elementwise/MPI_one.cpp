...
int main(int argc, char* argv[]) {
    ...
    if (world_rank == 0) {
        // 根进程分配足够大的内存
        MPI_Alloc_mem(total_size * sizeof(int), MPI_INFO_NULL, &global_result);
        base_ptr = global_result;
    } else {
        // 非根进程分配最小的内存
        MPI_Alloc_mem(local_size * sizeof(int), MPI_INFO_NULL, &base_ptr);
    }

    // 将本地结果复制到窗口
    MPI_Win win;
    MPI_Win_create(base_ptr, total_size * sizeof(int), sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD, &win);

    MPI_Win_fence(0, win);
    if (world_rank != 0) {
        MPI_Put(local_result.data(), local_size, MPI_INT, 0, 0, local_size, MPI_INT, win);
    } else {
        copy(local_result.begin(), local_result.end(), global_result);
    }
    MPI_Win_fence(0, win);

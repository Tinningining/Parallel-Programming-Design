...
vector<int> MIP_element_wise_binary_OpenMP(const vector<vector<uint32_t>>& vec, int world_rank, int world_size) {
    vector<int> local_result;
    size_t chunk_size = vec[0].size() / world_size;
    size_t start_idx = world_rank * chunk_size;
    size_t end_idx = (world_rank == world_size - 1) ? vec[0].size() : start_idx + chunk_size;
...
vector<int> MIP_element_wise_bitmap_OpenMP(const vector<vector<uint32_t>>& bitLists, int test_number, int world_rank, int world_size) {
    int size = bitLists[0].size()*sizeof(unsigned int)*8;
    vector<int> local_result;
    size_t chunk_size = size / world_size;
    size_t remainder = chunk_size % 32;
...
int main(int argc, char* argv[]) {
    ...
    for(int j = 0;j < 1;j ++){
        ...
        double element_ord_binary_OpenMP_Time=0.0;
        ...
        for(size_t i=0;i<Querys.size();i++){//
            ...
            if (world_rank == 0) {
                for (int k = 0; k < world_size; ++k) {
                    displs[k] = total_size;
                    total_size += all_sizes[k];
                }
            }

            vector<int> global_result(total_size);
            MPI_Gatherv(local_result.data(), local_size, MPI_INT, global_result.data(), all_sizes.data(), displs.data(), MPI_INT, 0, MPI_COMM_WORLD);
        ...
        for(size_t i=0;i<Querys.size();i++){//
            ...
            vector<int> local_result = MIP_element_wise_binary_OpenMP(Lists, world_rank, world_size);
            ...
            if (world_rank == 0) {
                gettimeofday(&end, NULL);
                double interval = (end.tv_sec - start.tv_sec) * 1000.0 + (end.tv_usec - start.tv_usec) / 1000.0;
                element_ord_binary_OpenMP_Time+=interval;
                // cout<<"binary第"<<i+1<<"条查询的结果为:"<<endl;
                // showresult(global_result);
                Lists.clear();
                bitLists.clear();
        ...
        if (world_rank == 0)
            cout << "binary_time_MPI_OpenMP:" << element_ord_binary_OpenMP_Time << "ms" << endl;
